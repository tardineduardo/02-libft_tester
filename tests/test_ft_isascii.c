...
void run_test(Test test, FailedTestNode **failed_tests, int *fail_count)
{
    ...
    int result = ft_isascii(test.input);
...
int main(void)
{
    ...
    printf("Comparing ft_isascii (result) with isascii/ctype.h (expected).\n");
    ...
    while (current != NULL)
    {
        printf(COLOR_RED "Failed: input = %d (%s) expected = %d | result = %d \n" COLOR_RESET, current->test.input, current->test.comment, isascii(current->test.input), ft_isascii(current->test.input));
