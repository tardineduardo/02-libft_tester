...
void	test_ft_isprint(int input, char *comment, int expected, int result, int *counter)
{
...
int	main(void)
{
    ...
    tests[0].result = ft_isprint(tests[0].input);
    tests[0].expected = isprint(tests[0].input);

    tests[1].input = 0;
    tests[1].comment = "zero";
    tests[1].result = ft_isprint(tests[1].input);
    tests[1].expected = isprint(tests[1].input);

    tests[2].input = 127;
    tests[2].comment = "bigger ascii number";
    tests[2].result = ft_isprint(tests[2].input);
    tests[2].expected = isprint(tests[2].input);

    tests[3].input = 178;
    tests[3].comment = "non-standard ASCII value";
    tests[3].result = ft_isprint(tests[3].input);
    tests[3].expected = isprint(tests[3].input);

    tests[4].input = 'a';
    tests[4].comment = "lowercase letter";
    tests[4].result = ft_isprint(tests[4].input);
    tests[4].expected = isprint(tests[4].input);
	...
	while (a < 7)
	{
		test_ft_isprint(tests[a].input, tests[a].comment, tests[a].result, tests[a].expected, counter);
		a++;
	}
